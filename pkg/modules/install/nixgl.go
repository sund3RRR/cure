...
func NewNixGLWrapper(nix Nix, gpu Gpu) *NixGLWrapper {
	return &NixGLWrapper{
		...
		workingDirs: []string{"bin"},
	}
}

func (n *NixGLWrapper) GetName() string {
...
func (n *NixGLWrapper) Apply(pkgPath types.Path, files []types.File) []types.File {
	binDir := filepath.Join(pkgPath.String(), "bin")
	...
	for _, f := range dirFiles {
		...
		content := strings.Join(
			[]string{
				"#!/usr/bin/env bash",
				fmt.Sprintf("%s %s \"$@\"", n.execPath, filepath.Join(pkgPath.String(), "bin", f.Name())),
			},
			"\n",
		)
